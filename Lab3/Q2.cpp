...
void Newton_Backward(vector<vector<double>> data,double x,int n){
    reverse(data[0].begin(),data[0].end());
    reverse(data[1].begin(),data[1].end());
    Newton_Forward(data,x,n);
}
int main(){
    cout<<"Part A\n";
    vector<vector<double>> data1 = {{-0.75, -0.5, -0.25, 0},{-0.07181250, 0.02475, 0.3349375, 1.101000}};
    Newton_Backward(data1,-0.3333,1);
    Newton_Backward(data1,-0.3333,2);
    Newton_Backward(data1,-0.3333,3);
    cout<<"\n Part B\n";
    vector<vector<double>> data2 = {{0.1, 0.2, 0.3, 0.4},{-0.62049958, -0.28398668, 0.00660095, 0.24842440}};
    Newton_Backward(data2,0.25,1);
    Newton_Backward(data2,0.25,2);
    Newton_Backward(data2,0.25,3);
    return 0;
}